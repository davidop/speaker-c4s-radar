...
> `$HOME/work/speaker-c4s-radar/speaker-c4s-radar/.github/get-shit-done/USER-PROFILE.md` exists). Skip the Read
...
```bash
PROFILE_CONTENT=$(cat "$HOME/work/speaker-c4s-radar/speaker-c4s-radar/.github/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Agent()` in parallel:
   ...
   ```
   Agent(
     prompt="First, read @$HOME/work/speaker-c4s-radar/speaker-c4s-radar/.github/agents/gsd-advisor-researcher.md for your role and instructions.
